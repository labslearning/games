...
class IcfesEngine {
  ...
  static generateLocalMasterclass(topic, lang) {
      ...
      return {
          ...
          theory: `[SISTEMA AISLADO DE DEEPSEEK]\n\nEl núcleo teórico de ${q.topic} evalúa tu competencia lectora de forma analítica. El ICFES no mide tu opinión personal sobre un tema, sino tu capacidad matemática para identificar qué dice el texto explícitamente (nivel literal), qué infiere estructuralmente (nivel global) y cómo se posiciona el autor lógicamente (nivel crítico).\n\nLa clave absoluta está en anclar cada respuesta a una oración o premisa que exista dentro de los límites físicos del texto.`,
          trap: "El ICFES suele emplear distractores de 'Conocimiento Previo': opciones que afirman verdades del mundo real que son correctas científicamente, pero que NO están mencionadas ni sustentadas en el texto en evaluación.",
          protocol: "1. Lee la pregunta antes que el texto para programar el escaneo mental.\n2. Identifica si te piden información local (un párrafo) o global (todo el texto).\n3. Descarta opciones que contengan generalizaciones absolutas ('siempre', 'todos', 'nunca') si el autor original no las usa.\n4. Selecciona la opción que tenga respaldo textual directo y comprobable.",
  ...
  static genGlobal(lang) {
    const texts = {
      es: {
        ...
        traps: [null, "El texto afirma exactamente lo contrario (nadie es justo por voluntad).", "El autor dice que el justo tomaría el mismo camino que el injusto si tuviera poder absoluto.", "El autor no dice que las leyes sean inútiles, sino que fuerzan al ser humano a actuar justamente."]
      },
      en: {
        topic: 'GLOBAL MEANING OF THE TEXT', text: `TEXT: "No one is just by will, but because they lack the power to commit injustices. If we grant both the just and the unjust the power to do whatever they want, we will catch the just man taking the same path as the unjust, following his own interests."\n\nWhat is the main thesis (global meaning) defended by the author?`, hint: "The thesis is the central idea the author is trying to prove.", micro: `The author argues that justice is not a natural desire, but an imposed restriction. Without punishment or oversight, everyone (even the 'just') would act selfishly and unjustly to benefit themselves. This unyielding human nature is the core of the text.`,
        ...
        traps: [null, "The text claims the exact opposite.", "The author says the just would act unjustly if given absolute power.", "The author doesn't say laws are useless; they force humans to act justly."]
      },
      fr: {
        topic: 'SENS GLOBAL DU TEXTE', text: `TEXTE : "Personne n'est juste par volonté, mais par manque de pouvoir de commettre des injustices. Si nous donnons au juste et à l'injuste le pouvoir de faire ce qu'ils veulent, nous surprendrons le juste empruntant le même chemin que l'injuste."\n\nQuelle est la thèse principale défendue par l'auteur ?`, hint: "La thèse est l'idée centrale que l'auteur tente de prouver.", micro: `L'auteur soutient que la justice n'est pas un désir naturel, mais une restriction imposée. Sans punition, tout le monde agirait de manière égoïste. Cette nature humaine inébranlable est le sens central du texte.`,
      ...
      de: {
        topic: 'GLOBALE BEDEUTUNG', text: `TEXT: "Niemand ist aus freiem Willen gerecht, sondern weil ihm die Macht fehlt, Unrecht zu tun. Wenn wir dem Gerechten und dem Ungerechten die Macht geben, zu tun, was sie wollen, werden wir den Gerechten auf demselben Weg wie den Ungerechten ertappen."\n\nWas ist die Hauptthese des Autors?`, hint: "Die These ist der Kerngedanke, den der Autor beweisen will.", micro: `Der Autor argumentiert, dass Gerechtigkeit kein natürlicher Wunsch ist, sondern eine auferlegte Einschränkung. Ohne Strafe würden alle egoistisch handeln. Diese menschliche Natur ist der Kern des Textes.`,
  ...
  static genCritical(lang) {
    const texts = {
      es: {
        topic: 'REFLEXIÓN CRÍTICA', text: `TEXTO: "Los recientes hallazgos demuestran que los videojuegos de acción mejoran la coordinación visomotora. Sin embargo, algunos sectores conservadores insisten, sin aportar datos empíricos, en prohibirlos bajo la excusa de que fomentan la violencia."\n\n¿Cuál es la actitud del autor hacia los "sectores conservadores"?`, hint: "Evalúa minuciosamente las palabras que usa el autor: 'insisten', 'sin aportar datos', 'excusa'.", micro: `El autor utiliza términos despectivos ('excusa', 'sin datos empíricos') para descalificar la postura de los sectores conservadores. Esta selección de vocabulario revela una actitud crítica y de claro rechazo hacia ellos, evidenciando una postura editorial muy marcada.`,
        opts: ["Crítica y de desaprobación, al considerarlos carentes de fundamentos lógicos.", "Objetiva e imparcial, limitándose a exponer ambas posturas científicas.", "Conciliadora, buscando un punto medio entre la ciencia y la moral.", "Indiferente, pues su enfoque está únicamente en la coordinación visomotora."],
        traps: [null, "No es imparcial; usa palabras con carga valorativa profunda como 'excusa'.", "No busca un punto medio, ataca abiertamente el argumento conservador sin ceder espacio.", "Si fuera indiferente, no los habría mencionado explícitamente para criticar su falta de datos."]
      },
      en: {
        topic: 'CRITICAL REFLECTION', text: `TEXT: "Recent findings show action video games improve visuomotor coordination. However, some conservative sectors insist, without providing empirical data, on banning them under the excuse that they promote violence."\n\nWhat is the author's attitude towards the "conservative sectors"?`, hint: "Evaluate the author's precise word choice: 'insist', 'without data', 'excuse'.", micro: `The author uses derogatory terms ('excuse', 'without empirical data') to discredit the conservative sectors. This vocabulary selection reveals a critical and disapproving attitude, evidencing a strong editorial stance.`,
        opts: ["Critical and disapproving, considering their arguments baseless.", "Objective and impartial, merely presenting both scientific sides.", "Conciliatory, seeking a middle ground between science and morals.", "Indifferent, focusing solely on visuomotor coordination."],
        traps: [null, "Not impartial; uses loaded words like 'excuse'.", "Does not seek a middle ground, openly attacks the conservative argument.", "If indifferent, the author wouldn't have mentioned them to criticize their lack of data."]
      },
      fr: {
        topic: 'RÉFLEXION CRITIQUE', text: `TEXTE : "Des découvertes récentes montrent que les jeux vidéo d'action améliorent la coordination visomotrice. Cependant, certains secteurs conservateurs insistent, sans fournir de données, pour les interdire sous l'excuse qu'ils favorisent la violence."\n\nQuelle est l'attitude de l'auteur envers les "secteurs conservateurs" ?`, hint: "Évaluez le choix des mots de l'auteur : 'insistent', 'sans données', 'excuse'.", micro: `L'auteur utilise des termes péjoratifs ('excuse', 'sans données') pour discréditer les secteurs conservateurs, révélant une attitude critique très marquée.`,
        opts: ["Critique et désapprobatrice, les jugeant sans fondement logique.", "Objective et impartiale, exposant simplement les deux côtés.", "Conciliante, cherchant un juste milieu entre science et morale.", "Indifférente, se concentrant uniquement sur la science visomotrice."],
        traps: [null, "Pas impartial ; utilise des mots chargés comme 'excuse'.", "Ne cherche pas de juste milieu, attaque l'argument sans céder.", "S'il était indifférent, il ne les aurait pas mentionnés pour les critiquer."]
      },
      de: {
        topic: 'KRITISCHE REFLEXION', text: `TEXT: "Jüngste Erkenntnisse zeigen, dass Action-Videospiele die visumotorische Koordination verbessern. Einige konservative Sektoren bestehen jedoch ohne empirische Daten darauf, sie unter dem Vorwand zu verbieten, sie würden Gewalt fördern."\n\nWie ist die Haltung des Autors gegenüber den "konservativen Sektoren"?`, hint: "Bewerten Sie die Wortwahl des Autors: 'bestehen', 'Vorwand', 'ohne Daten'.", micro: `Der Autor verwendet abwertende Begriffe ('Vorwand', 'ohne Daten'), um die konservativen Sektoren zu diskreditieren, was eine starke kritische und ablehnende Haltung offenbart.`,
        opts: ["Kritisch und ablehnend, hält ihre Argumente für unbegründet.", "Objektiv und unparteiisch, stellt nur beide Seiten dar.", "Versöhnlich, sucht einen Mittelweg zwischen Wissenschaft und Moral.", "Gleichgültig, konzentriert sich nur auf die visumotorische Koordination."],
        traps: [null, "Nicht unparteiisch; verwendet wertende Wörter wie 'Vorwand'.", "Sucht keinen Mittelweg, greift das Argument offen an.", "Wäre er gleichgültig, hätte er sie nicht kritisiert."]
      }
    };
  ...
  static genLiteral(lang) {
    const texts = {
      es: {
        topic: 'COMPRENSIÓN LITERAL', text: `TEXTO: "La ciudad es, en sí misma, un tema literario. Es la materia prima de los sueños y las pesadillas del hombre moderno. Sin embargo, no basta con hacer una mera nominación de calles o bares para ser un escritor urbano."\n\nSegún el texto explícito, ¿qué acción es considerada insuficiente para que alguien sea categorizado como un verdadero "escritor urbano"?`, hint: "Busca en la sintaxis del texto la acción que se señala como 'no basta' (es insuficiente).", micro: `El texto dice literalmente a nivel sintáctico: 'no basta con hacer una mera nominación de calles o bares'. Es una pregunta de extracción de información directa de Nivel 1. El estudiante no debe inferir, solo localizar la premisa.`,
        opts: ["Hacer una simple enumeración o nominación de calles y bares.", "Escribir exclusivamente sobre las pesadillas del hombre moderno.", "Vivir físicamente en una ciudad metropolitana.", "Rechazar la crítica literaria de la época moderna."],
        traps: [null, "El autor dice que la ciudad ES la materia prima de pesadillas, no que hacerlo sea insuficiente.", "El texto nunca menciona el lugar de residencia física real del autor.", "La crítica se menciona, pero no como un requisito rechazado o insuficiente."]
      },
      en: {
        topic: 'LITERAL COMPREHENSION', text: `TEXT: "The city is, in itself, a literary theme. It is the raw material of modern man's dreams and nightmares. However, simply naming streets or bars is not enough to be an urban writer."\n\nAccording explicitly to the text, what action is considered insufficient to be categorized as a true "urban writer"?`, hint: "Look in the text syntax for the action marked as 'not enough'.", micro: `The text explicitly states: 'simply naming streets or bars is not enough'. This is a direct Level 1 information extraction question. The student should not infer, just locate the premise.`,
        opts: ["Simply listing or naming streets and bars.", "Writing exclusively about modern man's nightmares.", "Physically living in a metropolitan city.", "Rejecting modern literary criticism."],
        traps: [null, "The author says the city IS the raw material, not that writing it is insufficient.", "The text does not mention the author's physical residence.", "Criticism is mentioned, but not as an insufficient requirement."]
      },
      fr: {
        topic: 'COMPRÉHENSION LITTÉRALE', text: `TEXTE : "La ville est, en soi, un thème littéraire. Elle est la matière première des rêves et cauchemars de l'homme. Cependant, il ne suffit pas de simplement nommer des rues ou des bars pour être un écrivain urbain."\n\nSelon le texte explicite, quelle action est insuffisante pour être considéré comme un "écrivain urbain" ?`, hint: "Cherchez dans le texte l'action qui 'ne suffit pas'.", micro: `Le texte dit explicitement : 'il ne suffit pas de simplement nommer des rues ou des bars'. Il s'agit d'une question d'extraction directe de Niveau 1. Il ne faut pas inférer, juste localiser.`,
        opts: ["Simplement énumérer ou nommer des rues et des bars.", "Écrire exclusivement sur les cauchemars de l'homme moderne.", "Vivre physiquement dans une grande métropole.", "Rejeter la critique littéraire moderne."],
        traps: [null, "L'auteur dit que la ville EST la matière première, pas que c'est insuffisant.", "Le texte ne mentionne pas la résidence physique.", "La critique n'est pas mentionnée comme une exigence insuffisante."]
      },
      de: {
        topic: 'LITERARISCHES VERSTÄNDNIS', text: `TEXT: "Die Stadt ist an sich ein literarisches Thema. Sie ist der Rohstoff für Träume und Albträume. Es reicht jedoch nicht aus, nur Straßen oder Bars zu benennen, um ein urbaner Schriftsteller zu sein."\n\nWelche Handlung reicht laut dem expliziten Text nicht aus, um als wahrer "urbaner Schriftsteller" zu gelten?`, hint: "Suchen Sie im Text nach der Handlung, die als 'nicht ausreichend' markiert ist.", micro: `Der Text besagt ausdrücklich: 'Es reicht jedoch nicht aus, nur Straßen oder Bars zu benennen'. Dies ist eine direkte Informationsextraktion der Stufe 1. Der Student soll nicht schlussfolgern, sondern lokalisieren.`,
        opts: ["Einfach nur Straßen und Bars aufzählen oder benennen.", "Ausschließlich über die Albträume des modernen Menschen schreiben.", "Physisch in einer Metropole leben.", "Moderne Literaturkritik ablehnen."],
        traps: [null, "Der Autor sagt, die Stadt IST der Rohstoff, nicht dass es unzureichend ist.", "Der Text erwähnt nicht den Wohnort des Autors.", "Kritik wird nicht als unzureichende Anforderung erwähnt."]
      }
    };
  ...
  static genArgumentacion(lang) {
    const texts = {
      es: {
        topic: 'ARGUMENTACIÓN Y PREMISAS', text: `TEXTO: "Si reducimos los impuestos a las grandes corporaciones, estas tendrán mucho más capital líquido. Si tienen más capital, generarán automáticamente más empleo a nivel nacional. Por lo tanto, para acabar con la crisis de desempleo, debemos reducir los impuestos ya mismo."\n\n¿Cuál es el supuesto subyacente (premisa implícita) más débil y cuestionable que asume el autor del argumento?`, hint: "Un supuesto es algo que el autor da por hecho como verdad absoluta sin probarlo.", micro: `El argumento asume ciegamente que el 'nuevo capital' se invertirá directamente en creación de empleo, ignorando deliberadamente otras realidades corporativas (que los directivos lo ahorren, paguen deudas pasadas, repartan dividendos o aumenten sus propios salarios). Esa es la falla argumentativa central.`,
        opts: ["Asume ingenuamente que todo el capital ahorrado se destinará exclusivamente a crear puestos de trabajo.", "Asume que las corporaciones pagan demasiados impuestos en la economía actual.", "Asume que el desempleo es el único y principal problema del país.", "Asume que los ciudadanos comunes no quieren pagar impuestos tampoco."],
        traps: [null, "El texto no opina sobre si los impuestos actuales son justos, altos o bajos, solo pide reducirlos.", "El autor propone una solución, pero no declara que sea el 'único' problema nacional.", "No se hace ninguna alusión al comportamiento de los ciudadanos naturales."]
      },
      en: {
        topic: 'ARGUMENTATION AND PREMISES', text: `TEXT: "If we lower corporate taxes, corporations will have much more liquid capital. If they have more capital, they will automatically create more national jobs. Therefore, to end the unemployment crisis, we must lower taxes right now."\n\nWhat is the weakest and most questionable underlying assumption (implicit premise) made by the author?`, hint: "An assumption is something taken for granted as absolute truth without proof.", micro: `The argument blindly assumes that the 'new capital' will be directly invested in job creation, deliberately ignoring other corporate realities (saving, paying debts, issuing dividends). That is the core argumentative flaw.`,
        opts: ["It naively assumes all saved capital will be exclusively used to create jobs.", "It assumes corporations currently pay too many taxes in the economy.", "It assumes unemployment is the country's single main problem.", "It assumes ordinary citizens don't want to pay taxes either."],
        traps: [null, "The text doesn't discuss if current taxes are fair, high, or low, just asks to lower them.", "The author proposes a solution, but doesn't state it's the 'only' national problem.", "There is no reference whatsoever to the behavior of ordinary citizens."]
      },
      fr: {
        topic: 'ARGUMENTATION ET PRÉMISSES', text: `TEXTE : "Si nous réduisons les impôts des sociétés, elles auront beaucoup plus de capital. Si elles ont plus de capital, elles créeront automatiquement des emplois. Par conséquent, pour mettre fin au chômage, réduisons les impôts maintenant."\n\nQuelle est la supposition (prémisse implicite) la plus faible de l'auteur ?`, hint: "Une supposition est une idée acceptée comme vérité absolue sans preuve.", micro: `L'argument suppose aveuglément que le 'nouveau capital' sera investi dans l'emploi, ignorant d'autres réalités (épargne, dividendes, dettes). C'est la faille centrale de l'argument.`,
        opts: ["Il suppose naïvement que tout le capital économisé sera utilisé pour créer des emplois.", "Il suppose que les sociétés paient actuellement trop d'impôts.", "Il suppose que le chômage est l'unique problème du pays.", "Il suppose que les citoyens ordinaires ne veulent pas payer d'impôts."],
        traps: [null, "Le texte ne dit pas si les impôts actuels sont justes ou élevés.", "Il propose une solution, mais ne déclare pas que c'est le 'seul' problème.", "Aucune mention n'est faite du comportement des citoyens ordinaires."]
      },
      de: {
        topic: 'ARGUMENTATION UND PRÄMISSEN', text: `TEXT: "Wenn wir die Unternehmenssteuern senken, haben sie mehr Kapital. Wenn sie mehr Kapital haben, schaffen sie automatisch mehr Arbeitsplätze. Um die Arbeitslosigkeit zu beenden, müssen wir die Steuern sofort senken."\n\nWas ist die schwächste und fragwürdigste Annahme (implizite Prämisse) des Autors?`, hint: "Eine Annahme ist etwas, das ohne Beweis als absolute Wahrheit vorausgesetzt wird.", micro: `Das Argument geht blind davon aus, dass 'neues Kapital' direkt in Arbeitsplätze investiert wird, und ignoriert andere Realitäten (Sparen, Schuldenabbau, Boni). Das ist der zentrale Argumentationsfehler.`,
        opts: ["Es wird naiv davon ausgegangen, dass das gesamte gesparte Kapital nur für Jobs verwendet wird.", "Es wird angenommen, dass Unternehmen derzeit zu viele Steuern zahlen.", "Es wird angenommen, dass Arbeitslosigkeit das einzige Problem des Landes ist.", "Es wird angenommen, dass auch normale Bürger keine Steuern zahlen wollen."],
        traps: [null, "Der Text erörtert nicht, ob die aktuellen Steuern fair, hoch oder niedrig sind.", "Der Autor schlägt eine Lösung vor, nennt es aber nicht das 'einzige' Problem.", "Es gibt keinerlei Bezug zum Verhalten gewöhnlicher Bürger."]
      }
    };
...
class DeepSeekEngine {
  ...
  static async generateQuestion(lang, forcedTopic = null, retries = 1) {
    ...
    const sysPrompt = `
      Eres un experto diseñador de exámenes ICFES de Colombia, enfocado 100% en LECTURA CRÍTICA a un nivel universitario.
      ...
      Inventa un fragmento de texto (entre 80 y 120 palabras) de tipo filosófico, literario o periodístico. Luego formula una pregunta profunda sobre el texto.
      Provee 4 opciones de respuesta, solo 1 correcta. Las otras 3 deben ser distractores sofisticados del ICFES (por ejemplo, inferencias exageradas, conocimiento previo no mencionado en el texto, o detalles locales que no responden al sentido global).
      ...
      EXTENSIÓN: La 'microclass' debe ser extensa y profesional (mínimo 100 palabras), diseccionando el texto a nivel sintáctico, semántico y pragmático para explicar por qué esa es la respuesta correcta y las demás son fallas lógicas.

      {
        "id": "${selectedTopic}",
        "topic": "Nombre de la competencia lectora",
        "text": "El fragmento de lectura (usa \\n para separar párrafos) y luego la pregunta...",
        "options": ["Opción Correcta", "Opción Trampa 1", "Opción Trampa 2", "Opción Trampa 3"],
        "correctIdx": 0,
        "hint": "Pista analítica para el estudiante",
        "microclass": "Explicación extensa, académica y rigurosa del análisis de lectura...",
    ...
    try {
      const response = await this.fetchWithTimeout(DEEPSEEK_URL, {
        ...
        body: JSON.stringify({ model: "deepseek-chat", messages: [{ role: "system", content: sysPrompt }], temperature: 0.7, max_tokens: 1500, response_format: { type: "json_object" } })
      }, 25000);
  ...
  static async generateMasterclass(topic, lang, retries = 2) {
    ...
    const sysPrompt = `
      Eres un Profesor de Lectura Crítica de élite preparando a un estudiante para la prueba de Estado ICFES.
      Genera una CLASE MAGISTRAL sobre la competencia: "${topic}".
      Language for the output MUST STRICTLY BE: ${targetLang}.
      Debe ser profundamente analítica, académica y extensa (entre 300 y 500 palabras), explicando cómo el ICFES evalúa esta competencia lectora, qué tipo de trampas pone y cómo usar la sintaxis y la semántica a favor.
      ...
      ESTRUCTURA EXACTA REQUERIDA:
      {
        ...
        "theory": "Explicación teórica extensa enfocada a la lectura y análisis de textos complejos.",
        "trap": "El tipo de distractor o trampa semántica típica del ICFES explicada a fondo.",
        "protocol": "1. Leer pregunta.\\n2. Identificar tipología textual.\\n3. Análisis pragmático.",
        "demoQuestion": {
           "text": "Minifragmento de lectura generado con una pregunta de inferencia profunda...",
           "options": ["A", "B", "C", "D"],
           "correctIdx": 0,
           "analysis": "Análisis extenso y académico de la interpretación correcta del texto."
        }
      }
    `;
    ...
    try {
      const response = await this.fetchWithTimeout(DEEPSEEK_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${DEEPSEEK_API_KEY}` },
        body: JSON.stringify({ model: "deepseek-chat", messages: [{ role: "system", content: sysPrompt }], temperature: 0.2, max_tokens: 1800, response_format: { type: "json_object" } })
      }, 30000); 
...
/* ============================================================
   🌍 5. DICCIONARIOS UI Y CONSEJOS (LECTURA CRÍTICA MASIVA)
============================================================ */
const DICT_UI = {
  es: {
      ...
      statsBtn: "TELEMETRÍA", theoryText: "MOTOR NEURONAL DE LECTURA ACTIVO. Conectado a DeepSeek. Se están generando textos continuos y discontinuos procedurales de gran profundidad académica. Cada reto evalúa tu capacidad de inferencia.",
  ...
  en: {
      start: "START READING IMMERSION", title: "ICFES CRITICAL READING LAB", scan: "SEMANTIC SCANNER", aiBtn: "AI TUTOR", time: "COGNITIVE TIMER", mastery: "Reading Mastery", btnCheck: "SYNTHESIZE RESPONSE", btnNext: "NEXT TEXT ➔", btnRetrySame: "RETRY ANALYSIS ➔", correctTitle: "PERFECT COGNITIVE ANALYSIS!", wrongTitle: "SEMANTIC RUPTURE", statsBtn: "TELEMETRY", theoryText: "NEURAL READING ENGINE ACTIVE. Hooked to DeepSeek. Generating procedural texts and deep inference challenges in real-time.", timeout: "ATTENTION COLLAPSE!", topic: "ACTIVE COMPETENCE", dashboard: "GLOBAL COGNITIVE DASHBOARD", avgTime: "Avg Reading Time", btnRetry: "PURGE MENTAL CACHE", aiSocraticBtn: "REQUEST AI MASTERCLASS", socraticModal: "AI HAS DETECTED FAILURES IN:", aiPraise: "PERFECT SYNTAX! NO COMPREHENSION FLAWS.", aiSelectTopic: "Select the competence to review:", aiClose: "CLOSE AI SESSION", downloadReport: "DOWNLOAD READING REPORT", loadingData: "ESTABLISHING DEEPSEEK SYNAPSES...", warmupTitle: "⚡ WARM-UP READING", warmupSub: "While AI synthesizes your main text..."
  },
  fr: {
      start: "DÉMARRER L'IMMERSION DE LECTURE", title: "LABO LECTURE CRITIQUE ICFES", scan: "SCANNER SÉMANTIQUE", aiBtn: "TUTEUR IA", time: "CHRONOMÈTRE COGNITIF", mastery: "Maîtrise de Lecture", btnCheck: "SYNTHÉTISER LA RÉPONSE", btnNext: "TEXTE SUIVANT ➔", btnRetrySame: "RÉESSAYER L'ANALYSE ➔", correctTitle: "ANALYSE COGNITIVE PARFAITE!", wrongTitle: "RUPTURE SÉMANTIQUE", statsBtn: "TÉLÉMÉTRIE", theoryText: "MOTEUR NEURONAL DE LECTURE ACTIF. Connecté à DeepSeek. Génération de textes procéduraux d'une grande profondeur académique.", timeout: "EFFONDREMENT DE L'ATTENTION!", topic: "COMPÉTENCE ACTIVE", dashboard: "TABLEAU DE BORD COGNITIF", avgTime: "Temps Moyen de Lecture", btnRetry: "PURGER LE CACHE MENTAL", aiSocraticBtn: "DEMANDER MASTERCLASS IA", socraticModal: "FAILLES DÉTECTÉES :", aiPraise: "SYNTAXE PARFAITE !", aiSelectTopic: "Sélectionnez la compétence :", aiClose: "FERMER LA SESSION IA", downloadReport: "TÉLÉCHARGER LE RAPPORT DE LECTURE", loadingData: "ÉTABLISSEMENT DES SYNAPSES...", warmupTitle: "⚡ LECTURE D'ÉCHAUFFEMENT", warmupSub: "Pendant que l'IA synthétise votre texte..."
  },
  de: {
      start: "LESEIMMERSION STARTEN", title: "ICFES KRITISCHES LESEN LABOR", scan: "SEMANTIK-SCANNER", aiBtn: "KI-TUTOR", time: "KOGNITIVER TIMER", mastery: "Lesebeherrschung", btnCheck: "ANTWORT SYNTHETISIEREN", btnNext: "NÄCHSTER TEXT ➔", btnRetrySame: "ANALYSE WIEDERHOLEN ➔", correctTitle: "PERFEKTE KOGNITIVE ANALYSE!", wrongTitle: "SEMANTISCHER BRUCH", statsBtn: "TELEMETRIE", theoryText: "NEURONALE LESE-KI AKTIV. Verbunden mit DeepSeek. Erzeugt prozedurale Texte und tiefe Inferenzherausforderungen.", timeout: "AUFMERKSAMKEITSKOLLAPS!", topic: "AKTIVE KOMPETENZ", dashboard: "GLOBALE KOGNITIVE TELEMETRIE", avgTime: "Durchschnittliche Lesezeit", btnRetry: "MENTALEN CACHE LÖSCHEN", aiSocraticBtn: "KI MASTERCLASS ANFORDERN", socraticModal: "FEHLER ERKANNT IN:", aiPraise: "PERFEKTE SYNTAX!", aiSelectTopic: "Wählen Sie die Kompetenz:", aiClose: "KI-SITZUNG SCHLIESSEN", downloadReport: "LESEBERICHT HERUNTERLADEN", loadingData: "AUFBAU DER SYNAPSEN...", warmupTitle: "⚡ AUFWÄRM-LEKTÜRE", warmupSub: "Während die KI deinen Haupttext synthetisiert..."
  }
};
...
// 💡 BANCO MASIVO DE 20 TIPS POR IDIOMA (ANTI-REPETICIÓN)
const TIPS_DB = {
  es: [
    ...
    "ANÁLISIS DE IMÁGENES: En textos discontinuos (infografías, cómics), el texto pequeño y las expresiones faciales de los personajes son tan importantes como el título principal.",
    "LA TRAMPA DE LO OBVIO: Si una opción te parece 'demasiado fácil y obvia' pero no está sustentada en el texto, probablemente es una trampa de conocimiento general.",
    "CONECTORES LÓGICOS: Presta atención a palabras como 'Sin embargo', 'Por lo tanto', 'A pesar de'. Estas palabras cambian por completo el sentido de la oración.",
    "TIPOLOGÍA TEXTUAL: Si el texto cuenta una historia con personajes, es Literario (Cuento/Novela). Si expone hechos y datos, es Informativo/Expositivo.",
    "NIVEL INFERENCIAL: A veces el texto no dice las cosas directamente. Si dice 'el suelo estaba mojado y había nubes grises', debes inferir que llovió, no buscar la palabra 'lluvia'.",
    "DOBLE NEGACIÓN: En español, una doble negación a menudo funciona como una afirmación oculta. Lee despacio esas oraciones.",
    "TRAMPA DE GENERALIZACIÓN: Si el texto habla sobre 'algunos perros', la respuesta correcta NUNCA puede decir 'todos los animales'.",
    "TEXTOS ARGUMENTATIVOS: El autor siempre quiere convencerte de algo. Identifica de qué quiere convencerte y tendrás el 50% de la pregunta ganada.",
    "VOZ DEL AUTOR: Diferencia siempre entre lo que el autor narra que pasó, y lo que el autor 'opina' sobre lo que pasó.",
    "PREGUNTAS DE VOCABULARIO: Nunca definas una palabra por lo que sabes de ella en la calle, defínela por el sentido que cobra dentro del contexto de esa oración exacta.",
    "IRONÍA Y SARCASMO: Cuidado con los textos literarios y columnas de opinión. A veces el autor dice algo positivo para burlarse de una situación negativa.",
    "PRONOMBRES CLAVE: Rastrear a qué se refieren los pronombres (él, eso, aquello) te salva de perder el hilo conductor en párrafos muy largos.",
    "TEXTOS CIENTÍFICOS: Aquí no importan las opiniones, importan los datos, la hipótesis inicial y la conclusión final.",
    "OPCIONES EXAGERADAS: El ICFES ama poner una opción que es parcialmente correcta pero que añade un detalle exagerado que la arruina por completo.",
    "REGLA DE ORO: Si dos opciones parecen correctas, pregúntate: ¿cuál de las dos responde directamente a lo que me están preguntando y no a otra cosa?"
  ],
  en: [
    ...
    "IMAGE ANALYSIS: In discontinuous texts (infographics, comics), small text and facial expressions are as important as the main title.",
    "THE OBVIOUS TRAP: If an option seems 'too easy' but isn't supported by the text, it's likely a general knowledge trap.",
    "LOGICAL CONNECTORS: Pay attention to words like 'However', 'Therefore', 'Despite'. These completely change the sentence's meaning.",
    "TEXT TYPOLOGY: If it tells a story with characters, it's Literary. If it presents facts, it's Informative/Expository.",
    "INFERENTIAL LEVEL: Sometimes texts don't state things directly. If it says 'the ground was wet and clouds were gray', infer it rained.",
    "DOUBLE NEGATIVES: A double negative often functions as a hidden positive. Read those sentences very slowly.",
    "GENERALIZATION TRAP: If the text talks about 'some dogs', the correct answer can NEVER say 'all animals'.",
    "ARGUMENTATIVE TEXTS: The author always wants to convince you of something. Identify what that is to win 50% of the battle.",
    "AUTHOR'S VOICE: Always differentiate between what the author narrates happened, and what the author 'thinks' about what happened.",
    "VOCABULARY QUESTIONS: Never define a word by its street definition; define it by the meaning it takes on within that exact sentence context.",
    "IRONY AND SARCASM: Beware of literary texts and opinion columns. Sometimes the author says something positive to mock a negative situation.",
    "KEY PRONOUNS: Tracking what pronouns refer to saves you from losing the main thread in long paragraphs.",
    "SCIENTIFIC TEXTS: Opinions don't matter here; what matters is the data, the initial hypothesis, and the final conclusion.",
    "EXAGGERATED OPTIONS: The ICFES loves putting an option that is partially correct but adds an exaggerated detail that ruins it.",
    "GOLDEN RULE: If two options seem correct, ask yourself: which one directly answers the question being asked and not something else?"
  ],
  fr: [
    "ASTUCE ICFES : Lisez toujours la question AVANT le texte pour scanner efficacement l'information.",
    "ATTENTION AU CONTEXTE : La bonne réponse se trouve DANS le texte, pas dans vos connaissances générales.",
    "MÉFIEZ-VOUS DES ABSOLUS : Évitez les mots comme 'toujours', 'jamais', 'tous', sauf si l'auteur est radical.",
    "NIVEAU CRITIQUE : L'intention de l'auteur se cache souvent dans les adjectifs qu'il utilise (péjoratifs ou élogieux).",
    "STRATÉGIE : Dans les textes philosophiques, identifiez la thèse principale et les arguments qui la soutiennent.",
    "ANALYSE D'IMAGES : Dans les infographies, les petits textes et expressions faciales sont cruciaux.",
    "PIÈGE DE L'ÉVIDENCE : Une option 'trop évidente' non soutenue par le texte est souvent un piège de culture générale.",
    "CONNECTEURS LOGIQUES : 'Cependant', 'Donc', 'Malgré' changent tout le sens d'une phrase.",
    "TYPOLOGIE : Une histoire avec des personnages est littéraire. Des faits sont informatifs.",
    "NIVEAU INFÉRENTIEL : Si le sol est mouillé et le ciel gris, déduisez qu'il a plu, ne cherchez pas le mot 'pluie'.",
    "DOUBLE NÉGATION : Lisez lentement les phrases avec une double négation, elles cachent souvent une affirmation.",
    "PIÈGE DE GÉNÉRALISATION : Si le texte parle de 'certains chiens', la réponse ne peut pas être 'tous les animaux'.",
    "TEXTES ARGUMENTATIFS : L'auteur veut vous convaincre de quelque chose. Identifiez quoi pour faciliter la réponse.",
    "VOIX DE L'AUTEUR : Différenciez les faits racontés des opinions de l'auteur sur ces faits.",
    "VOCABULAIRE : Définissez un mot selon son contexte strict dans la phrase, pas son sens courant.",
    "IRONIE ET SARCASME : Attention, l'auteur peut dire quelque chose de positif pour se moquer d'une situation.",
    "PRONOMS CLÉS : Suivre à quoi se réfèrent les pronoms évite de perdre le fil dans les longs paragraphes.",
    "TEXTES SCIENTIFIQUES : Les opinions n'ont pas d'importance, seules les données et conclusions comptent.",
    "OPTIONS EXAGÉRÉES : Attention aux réponses partiellement justes mais ruinées par un détail exagéré.",
    "RÈGLE D'OR : Si deux options semblent correctes, choisissez celle qui répond directement à la question posée."
  ],
  de: [
    "ICFES-TRICK: Lesen Sie IMMER die Frage VOR dem Text, um das Gehirn auf die Suche zu fokussieren.",
    "KONTEXT VORSICHT: Die richtige Antwort basiert fast immer auf dem Text, nicht auf Ihrem Vorwissen.",
    "VORSICHT VOR ABSOLUTEN: Misstrauen Sie Wörtern wie 'immer', 'nie', 'alle', es sei denn, der Autor ist explizit radikal.",
    "KRITISCHES NIVEAU: Die Absicht des Autors zeigt sich oft in seinen Adjektiven (abwertend oder lobend).",
    "STRATEGIE: Suchen Sie in philosophischen Texten nach der Hauptthese und den unterstützenden Argumenten.",
    "BILDANALYSE: In Infografiken und Comics sind Kleingedrucktes und Gesichtsausdrücke so wichtig wie der Titel.",
    "DIE OFFENSICHTLICHE FALLE: Eine 'zu einfache' Option ohne Textbeleg ist oft eine Allgemeinwissensfalle.",
    "LOGISCHE VERBINDUNGEN: Achten Sie auf 'Jedoch', 'Daher', 'Trotz'. Sie ändern die Bedeutung völlig.",
    "TEXTTYPOLOGIE: Eine Geschichte mit Charakteren ist literarisch. Fakten und Daten sind informativ.",
    "INFERENTIELLES NIVEAU: Wenn der Boden nass und der Himmel grau ist, folgern Sie, dass es geregnet hat.",
    "DOPPELTE VERNEINUNG: Eine doppelte Verneinung wirkt oft wie eine versteckte Bejahung. Langsam lesen.",
    "VERALLGEMEINERUNGSFALLE: Wenn der Text von 'einigen Hunden' spricht, kann die Antwort nicht 'alle Tiere' lauten.",
    "ARGUMENTATIVE TEXTE: Der Autor will Sie überzeugen. Finden Sie heraus, wovon, um die Frage leichter zu lösen.",
    "AUTORENSTIMME: Unterscheiden Sie zwischen erzählten Fakten und der Meinung des Autors dazu.",
    "VOKABELFRAGEN: Definieren Sie ein Wort niemals nach seinem Alltagssinn, sondern nach seinem genauen Kontext im Satz.",
    "IRONIE UND SARKASMUS: Der Autor sagt manchmal etwas Positives, um eine negative Situation zu verspotten.",
    "SCHLÜSSELPRONOMEN: Verfolgen Sie Pronomen (er, es, das), um in langen Absätzen nicht den Faden zu verlieren.",
    "WISSENSCHAFTLICHE TEXTE: Meinungen zählen hier nicht, nur Daten, Hypothesen und Schlussfolgerungen.",
    "ÜBERTRIEBENE OPTIONEN: Vorsicht vor Optionen, die teilweise richtig sind, aber durch ein übertriebenes Detail falsch werden.",
    "GOLDENE REGEL: Wenn zwei Optionen richtig erscheinen, fragen Sie sich: Welche beantwortet direkt die gestellte Frage?"
  ]
};
...
/* ============================================================
   🎥 7. NÚCLEO 3D AVANZADO (RED NEURONAL LECTORA - ESTÁTICO)
============================================================ */
    ...
    <>
      ...
      {scannerActive && (
         <mesh ref={scanPlaneRef} rotation={[-Math.PI/2, 0, 0]}>
            ...
            <meshBasicMaterial color="#ff00ff" transparent opacity={0.1} side={THREE.DoubleSide} blending={THREE.AdditiveBlending} depthWrite={false} />
         </mesh>
      )}
...
const MarkdownParser = ({ text }) => {
    const htmlContent = useMemo(() => {
        ...
        let parsed = text;
        // Tipografía ampliada masivamente para lectura cómoda en móviles
        parsed = parsed.replace(/### (.*)/g, '<h3 style="color:#00f2ff; margin-top:clamp(20px, 4vw, 30px); border-bottom:1px solid #00f2ff; padding-bottom:5px; font-size: clamp(22px, 5vw, 32px);">$1</h3>');
        parsed = parsed.replace(/## (.*)/g, '<h2 style="color:#ffea00; margin-top:clamp(15px, 3vw, 25px); font-size: clamp(24px, 5.5vw, 36px);">$1</h2>');
    ...
    return <div dangerouslySetInnerHTML={htmlContent} style={{ color: '#fff', fontSize: 'clamp(18px, 4.5vw, 28px)', lineHeight: '1.7', fontFamily: 'sans-serif' }} />;
};
        ...
        <div style={{ padding: 'clamp(10px, 3vw, 30px)', width: '100%', boxSizing: 'border-box' }}>
           ...
           <div style={{ display: 'grid', gap: 'clamp(20px, 4vw, 30px)', marginTop: '40px' }}>
              <div style={{ borderLeft: '6px solid #00f2ff', padding: 'clamp(15px, 3vw, 30px)', background: 'rgba(0,242,255,0.05)', borderRadius: '0 15px 15px 0' }}>
                 <h3 style={{color: '#00f2ff', marginTop: 0, fontSize: 'clamp(20px, 4.5vw, 30px)', display: 'flex', alignItems: 'center', gap:'10px'}}>📚 NÚCLEO TEÓRICO Y ANÁLISIS</h3>
              ...
              <div style={{ borderLeft: '6px solid #f00', padding: 'clamp(15px, 3vw, 30px)', background: 'rgba(255,0,0,0.05)', borderRadius: '0 15px 15px 0' }}>
                 <h3 style={{color: '#f00', marginTop: 0, fontSize: 'clamp(20px, 4.5vw, 30px)', display: 'flex', alignItems: 'center', gap:'10px'}}>⚠️ TRAMPA COGNITIVA ICFES</h3>
              ...
              <div style={{ borderLeft: '6px solid #ffea00', padding: 'clamp(15px, 3vw, 30px)', background: 'rgba(255,234,0,0.05)', borderRadius: '0 15px 15px 0' }}>
                 <h3 style={{color: '#ffea00', marginTop: 0, fontSize: 'clamp(20px, 4.5vw, 30px)', display: 'flex', alignItems: 'center', gap:'10px'}}>⚙️ PROTOCOLO SEMÁNTICO INFALIBLE</h3>
           ...
           {classData.demoQuestion && (
               <div style={{ marginTop: 'clamp(30px, 6vw, 50px)', border: '3px solid #0f0', borderRadius: '15px', padding: 'clamp(15px, 4vw, 40px)', background: 'rgba(0,20,5,0.95)', boxShadow: '0 0 40px rgba(0,255,0,0.15)', boxSizing: 'border-box' }}>
                   <h3 style={{color: '#0f0', textAlign: 'center', marginTop: 0, fontSize: 'clamp(22px, 5vw, 32px)'}}>📖 SIMULACIÓN DE LECTURA</h3>
                   ...
                   <div style={{ color: '#fff', fontSize: 'clamp(20px, 5vw, 32px)', lineHeight: '1.7', background: 'rgba(255,255,255,0.05)', padding: 'clamp(15px, 3vw, 30px)', borderRadius: '10px', whiteSpace: 'pre-wrap' }}>
                   ...
                   <div style={{ marginTop: '30px', display: 'flex', flexDirection: 'column', gap: 'clamp(15px, 3vw, 20px)' }}>
                      {classData.demoQuestion.options.map((opt, idx) => {
                          ...
                          return (
                              <div key={idx} style={{ padding: 'clamp(15px, 3vw, 25px)', border: isCorrect ? '3px solid #0f0' : '2px solid #333', background: isCorrect ? 'rgba(0,255,0,0.1)' : 'transparent', color: isCorrect ? '#0f0' : '#aaa', borderRadius: '10px', fontSize: 'clamp(18px, 4.5vw, 28px)', fontWeight: isCorrect ? 'bold' : 'normal', wordWrap: 'break-word', minHeight: '80px', display: 'flex', alignItems: 'center' }}>
                   ...
                   <div style={{ marginTop: '30px', padding: 'clamp(15px, 3vw, 30px)', background: 'rgba(0,255,0,0.05)', color: '#0f0', borderRadius: '10px', borderLeft: '6px solid #0f0', fontSize: 'clamp(18px, 4.5vw, 28px)', lineHeight: '1.8' }}>
                       <strong style={{fontSize: 'clamp(20px, 4.5vw, 30px)'}}>ANÁLISIS DEL RESULTADO:</strong><br/><br/>
...
function GameApp() {
  ...
  // ⏱️ 4 MINUTOS EXACTOS POR PREGUNTA COMO ORDENÓ EL COMANDANTE
  const MAX_TIME = 240; 
    ...
    <>
      <style>{`
        * { -webkit-tap-highlight-color: transparent; }
        .hud-btn { padding: clamp(12px, 3vw, 20px) clamp(15px, 4vw, 40px); background: #00f2ff; color: #000; font-weight: 900; font-size: clamp(16px, 4vw, 26px); cursor: pointer; border-radius: 8px; border: none; font-family: 'Orbitron', monospace; transition: 0.2s; box-shadow: 0 0 20px rgba(0,242,255,0.4); text-transform: uppercase; user-select: none; }
        ...
        .opt-btn { display: block; width: 100%; margin: 10px 0; padding: clamp(15px, 4vw, 25px); background: rgba(0,20,40,0.8); border: 2px solid #555; color: #fff; font-size: clamp(18px, 4.5vw, 28px); border-radius: 10px; cursor: pointer; text-align: left; transition: 0.2s; font-family: 'Orbitron'; line-height: 1.6; user-select: none; min-height: 80px; }
      ...
      <main style={{ position:'absolute', inset:0, overflow:'hidden', background:'#000', fontFamily:'Orbitron, sans-serif', padding: 'env(safe-area-inset-top) env(safe-area-inset-right) env(safe-area-inset-bottom) env(safe-area-inset-left)', boxSizing: 'border-box' }}>
        ...
        {(phase === "BOOT" || phase === "THEORY") && (
          <section style={{ position:'absolute', inset:0, zIndex:3000, background:'#000510', display:'flex', flexDirection:'column', justifyContent:'center', alignItems:'center', padding:'clamp(15px, 4vw, 20px)', boxSizing: 'border-box' }}>
            ...
            {phase === "THEORY" && <p style={{color:'#fff', fontSize:'clamp(16px, 4vw, 26px)', maxWidth:'800px', textAlign:'center', marginBottom:'clamp(30px, 6vw, 40px)', lineHeight:'1.6', borderLeft: '4px solid #ffea00', paddingLeft: 'clamp(10px, 3vw, 20px)'}}>{UI.theoryText}</p>}
          ...
          <>
            ...
            <nav style={{ position: 'absolute', top: 'max(15px, env(safe-area-inset-top))', left: 'clamp(10px, 3vw, 20px)', right: 'clamp(10px, 3vw, 20px)', display: 'flex', justifyContent: 'space-between', zIndex: 500, flexWrap:'wrap', gap:'10px' }}>
              ...
              {phase !== "STATS" && phase !== "LOADING" && (
                 <button className="hud-btn" style={{background:'rgba(30,30,30,0.8)', color:'#00f2ff', border:'1px solid #00f2ff', padding:'8px 15px', fontSize:'clamp(12px, 3vw, 16px)', boxShadow:'none'}} onClick={openTelemetry}>📊 {UI.statsBtn}</button>
              )}
            </nav>

            {/* SEÑALADOR DE DEEPSEEK NEURAL MATRIX */}
            {phase === "GAME" && (
               <div style={{position: 'absolute', bottom: 'max(15px, env(safe-area-inset-bottom))', right: 'clamp(10px, 3vw, 15px)', color: currentQData?.isAi ? '#ff00ff' : '#00f2ff', fontSize: 'clamp(10px, 2.5vw, 14px)', fontWeight: 'bold', zIndex: 100, textShadow: currentQData?.isAi ? '0 0 10px #ff00ff' : 'none', letterSpacing: '1px', textAlign: 'right'}}>
            ...
            {(phase === "GAME" || phase === "WARMUP") && (
                <div style={{position:'absolute', top:'clamp(70px, calc(env(safe-area-inset-top) + 70px), 120px)', left:'50%', transform:'translateX(-50%)', width:'95%', maxWidth:'800px', zIndex:100, display:'flex', flexDirection:'column', alignItems:'center'}}>
                    ...
                    <div style={{color: timeLeft > 30 ? '#00f2ff' : (timeLeft > 15 ? '#ffaa00' : '#f00'), fontSize:'clamp(20px, 5vw, 32px)', fontWeight:'bold', marginBottom:'10px', textShadow: timeLeft <= 15 ? '0 0 10px #f00' : 'none'}} className={timeLeft <= 15 ? 'hud-pulse' : ''}>
                    ...
                    {!scannerActive && phase === "GAME" && (
                        <button className="hud-btn" style={{background:'rgba(0,255,0,0.1)', border:'1px solid #0f0', color:'#0f0', marginTop:'15px', fontSize:'clamp(12px, 3vw, 18px)', padding:'8px 15px', backdropFilter:'blur(10px)'}} onClick={toggleScanner}>
                            👁️ {UI.scan} (-50 XP)
                        </button>
                    )}
                </div>
            ...
            {(phase === "GAME" || phase === "WARMUP") && currentQ && (
              <div style={{ position:'absolute', top:'clamp(160px, 25vh, 220px)', left:'50%', transform:'translateX(-50%)', width: '95%', maxWidth:'1000px', zIndex:100, boxSizing: 'border-box' }}>
                <article className="glass-panel" style={{maxHeight:'82dvh', overflowY:'auto', borderColor: phase === "WARMUP" ? '#ffaa00' : '#00f2ff'}}>
                  ...
                  {scannerActive && (
                      <div className="hud-pulse" style={{background:'rgba(0,242,255,0.1)', borderLeft:'4px solid #00f2ff', padding:'clamp(15px, 4vw, 20px)', margin:'20px 0', color:'#00f2ff', fontSize:'clamp(16px, 4vw, 24px)', fontWeight:'bold'}}>
                  ...
                  <div style={{display:'flex', justifyContent:'center', marginTop:'clamp(20px, 5vw, 40px)'}}>
                      <button className="hud-btn" style={{width:'100%', height:'clamp(60px, 10vw, 90px)', fontSize:'clamp(18px, 4.5vw, 32px)', background: phase === "WARMUP" ? '#ffaa00' : '#00f2ff'}} disabled={selectedOpt === null} onClick={submitAnswer}>{UI.btnCheck}</button>
                  </div>
                </article>
              </div>
            )}
            ...
            {phase === "CORRECT" && (
              <section style={{ position:'absolute', inset:0, zIndex:2000, background:'rgba(0,40,10,0.95)', display:'flex', flexDirection:'column', justifyContent:'center', alignItems:'center', backdropFilter:'blur(10px)', padding:'clamp(15px, 4vw, 20px)', textAlign:'center', boxSizing: 'border-box' }}>
                  <h1 className="hud-pulse" style={{color:'#0f0', fontSize:'clamp(28px, 7vw, 90px)', textShadow:'0 0 40px #0f0', margin:0, lineHeight: 1.1}}>{UI.correctTitle}</h1>
                  <p style={{color:'#fff', fontSize:'clamp(22px, 5vw, 40px)', marginTop:'20px'}}>XP: <span style={{color:'#0f0', fontWeight:'bold'}}>+{hintUsed ? '50' : '200'}</span></p>
                  <p style={{color:'#aaa', fontSize:'clamp(16px, 4vw, 24px)'}}>{UI.time}: {MAX_TIME - timeLeft}s</p>
            ...
            {phase === "MICROCLASS" && (
              <section style={{ position:'absolute', inset:0, zIndex:2000, background:'rgba(40,0,0,0.95)', display:'flex', flexDirection:'column', justifyContent:'center', alignItems:'center', padding:'clamp(10px, 3vw, 20px)', backdropFilter:'blur(10px)', boxSizing: 'border-box' }}>
                  <article className="glass-panel" style={{borderColor:'#f00', maxWidth:'1000px', width:'100%', textAlign:'left', boxShadow:'0 0 50px rgba(255,0,255,0.3)', maxHeight:'90dvh', overflowY:'auto'}}>
                      <h1 style={{color:'#f00', fontSize:'clamp(24px, 6vw, 60px)', marginBottom:'20px', textAlign:'center', borderBottom:'2px solid #f00', paddingBottom:'10px', lineHeight: 1.1}}>
                          ⚠️ {timeLeft === 0 ? UI.timeout : UI.wrongTitle}
                      </h1>
                      
                      {currentQData?.trapExplanations && currentQData.trapExplanations[selectedOpt] && (
                          <div style={{color:'#ffaa00', fontSize:'clamp(16px, 4vw, 26px)', marginTop:'15px', padding:'clamp(15px, 3vw, 20px)', background:'rgba(255,170,0,0.1)', borderLeft:'4px solid #ffaa00', fontWeight:'bold'}}>
                              {currentQData.trapExplanations[selectedOpt]}
                          </div>
                      )}

                      <div style={{color:'#fff', fontSize:'clamp(16px, 4vw, 26px)', lineHeight:'1.7', background:'rgba(0,0,0,0.6)', padding:'clamp(15px, 4vw, 30px)', borderRadius:'10px', whiteSpace:'pre-wrap', borderLeft:'4px solid #00f2ff', marginTop:'20px'}}>
                          <MarkdownParser text={currentQData?.microclass} />
                      </div>
                      
                      <div style={{display:'flex', justifyContent:'center'}}>
                          <button className="hud-btn" style={{marginTop:'clamp(20px, 5vw, 40px)', background:'#ffea00', color:'#000', width:'100%', height:'clamp(60px, 10vw, 90px)', fontSize:'clamp(18px, 4.5vw, 28px)'}} onClick={handleNextPhase}>{UI.btnRetrySame}</button>
                      </div>
                  </article>
              </section>
            )}
            ...
            {phase === "STATS" && !showAiModal && (
              <section style={{ position:'absolute', inset:0, zIndex:2000, background:'rgba(0,10,30,0.98)', display:'flex', flexDirection:'column', justifyContent:'center', alignItems:'center', padding:'clamp(10px, 3vw, 20px)', boxSizing: 'border-box' }}>
                  <article className="glass-panel" style={{maxWidth:'1000px', width:'100%', maxHeight:'95dvh', overflowY:'auto'}}>
                      <h2 style={{color:'#00f2ff', textAlign:'center', fontSize:'clamp(24px, 6vw, 45px)', borderBottom:'2px solid #00f2ff', paddingBottom:'15px', margin:0}}>{UI.dashboard}</h2>
                      ...
                      <div style={{display:'flex', justifyContent:'space-around', flexWrap:'wrap', margin:'clamp(20px, 5vw, 40px) 0', gap:'clamp(10px, 3vw, 20px)'}}>
                          <div style={{textAlign:'center', color:'#fff', background:'rgba(255,255,255,0.05)', padding:'clamp(15px, 3vw, 20px)', borderRadius:'15px', flex:'1 1 140px'}}>
                              <div style={{fontSize:'clamp(32px, 8vw, 70px)', fontWeight:'bold', color: stats.totalQ === 0 ? '#aaa' : (stats.correctQ/stats.totalQ > 0.6 ? '#0f0' : '#ff0055')}}>
                                  {stats.totalQ > 0 ? Math.round((stats.correctQ/stats.totalQ)*100) : 0}%
                              </div>
                              <div style={{fontSize:'clamp(12px, 3vw, 20px)', letterSpacing:'1px', color:'#aaa'}}>{UI.mastery}</div>
                          </div>
                          <div style={{textAlign:'center', color:'#fff', background:'rgba(255,255,255,0.05)', padding:'clamp(15px, 3vw, 20px)', borderRadius:'15px', flex:'1 1 140px'}}>
                              <div style={{fontSize:'clamp(32px, 8vw, 70px)', fontWeight:'bold', color:'#ffea00'}}>
                                  {stats.totalQ > 0 ? Math.round(stats.totalTime/stats.totalQ) : 0}s
                              </div>
                              <div style={{fontSize:'clamp(12px, 3vw, 20px)', letterSpacing:'1px', color:'#aaa'}}>{UI.avgTime}</div>
                          </div>
                      </div>
                      ...
                      <div style={{display:'flex', justifyContent:'center', marginBottom:'clamp(20px, 5vw, 30px)'}}>
                         <button 
                            className="hud-btn" 
                            style={{background: failedTopics.length > 0 ? '#ff00ff' : '#0f0', color: failedTopics.length > 0 ? '#fff' : '#000', width: '100%', maxWidth: '600px', boxShadow: failedTopics.length > 0 ? '0 0 30px #ff00ff' : '0 0 30px #0f0', fontSize: 'clamp(16px, 4vw, 24px)'}} 
                          ...
                          {Object.keys(stats.topics).map(topicId => {
                              ...
                              return (
                                  <div key={topicId} className={isFailed ? 'hud-pulse' : ''} style={{background:'rgba(0,0,0,0.5)', padding:'clamp(10px, 3vw, 20px)', borderRadius:'10px', border: isFailed ? '2px solid #f00' : '1px solid #333'}}>
                                      <div style={{color: isFailed ? '#f00' : '#00f2ff', fontWeight:'bold', marginBottom:'10px', fontSize:'clamp(12px, 3vw, 18px)'}}>
                                      ...
                                      <div style={{color:'#aaa', fontSize:'clamp(12px, 3vw, 16px)', marginTop:'8px', display:'flex', justifyContent:'space-between'}}>
                      ...
                      <div style={{display:'flex', justifyContent:'center', marginTop:'clamp(30px, 6vw, 50px)', gap:'clamp(10px, 3vw, 20px)', flexWrap:'wrap'}}>
                          <button className="hud-btn" style={{flex:'1 1 200px', background:'transparent', border:'2px solid #00f2ff', color:'#00f2ff', fontSize: 'clamp(14px, 3.5vw, 20px)'}} onClick={downloadReport}>📄 {UI.downloadReport}</button>
                          <button className="hud-btn" style={{flex:'1 1 200px', fontSize: 'clamp(14px, 3.5vw, 20px)'}} onClick={closeTelemetry}>VOLVER A LA MISIÓN</button>
                          <button className="hud-btn" style={{flex:'1 1 200px', background:'transparent', border:'2px solid #f00', color:'#f00', fontSize: 'clamp(14px, 3.5vw, 20px)'}} onClick={() => { window.localStorage.removeItem('icfes_reading_telemetry_v1'); window.location.reload(); }}>{UI.btnRetry}</button>
                      </div>
                  </article>
              </section>
            )}
