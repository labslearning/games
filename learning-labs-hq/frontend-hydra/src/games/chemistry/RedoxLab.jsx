...
export default function RedoxLab() {
  ...
  // Pantallas de inicio y finalización
      ...
      {phase === "THEORY" && (
        <div style={ui.overlay}>
          <div style={ui.dialogBox('#00f2ff')}>
            <h2 style={{color: '#00f2ff', letterSpacing:'3px', fontSize:'clamp(20px, 6vw, 35px)', margin: 0}}>{dict.ui.theoryTitle}</h2>
            <p style={{color:'#fff', fontSize:'clamp(16px, 4.5vw, 22px)', lineHeight:'1.5', margin: 0}}>{dict.theory[missionIdx]}</p>
            <button style={ui.nextBtn('#00f2ff')} onClick={() => { setPhase("DIAGNOSIS"); safeSpeak(config.isGalvanic ? dict.ai.boss : dict.ai.start, langCode); }}>{dict.ui.theoryBtn}</button>
          </div>
        </div>
      )}
      ...
      {phase === "DIAGNOSIS" && (
        <div style={ui.overlay}>
          <div style={ui.dialogBox(config.isGalvanic ? '#ff0055' : '#ffea00')}>
            <h2 style={{color: config.isGalvanic ? '#ff0055' : '#ffea00', letterSpacing:'3px', fontSize:'clamp(20px, 6vw, 35px)', margin: 0}}>{dict.ui.diagTitle}</h2>
            {!config.isGalvanic ? (
              <>
                <p style={{color:'#fff', fontSize:'clamp(16px, 4.5vw, 22px)', margin: 0}}>{dict.elements[missionIdx]} {dict.ui.diagQ1} <b style={{color:'#ffea00'}}>{cores[0]?.start}</b> {dict.ui.diagQ2} <b style={{color:'#0f0'}}>{cores[0]?.target}</b>.</p>
                <p style={{color:'#aaa', fontSize:'clamp(14px, 4vw, 18px)', margin: 0}}>{dict.ui.diagQ3}</p>
              ...
              <>
                <p style={{color:'#fff', fontSize:'clamp(16px, 4.5vw, 22px)', margin: 0}}>{dict.ui.diagGalvanic}</p>
                <div style={ui.btnGroup}>
      ...
      {phase === "AI_QUESTION" && (
        <div style={ui.overlay}>
          <div style={ui.dialogBox('#ff00ff')}>
            <h2 style={{color:'#ff00ff', letterSpacing:'3px', fontSize:'clamp(20px, 6vw, 35px)', margin: 0}}>{microClassActive ? dict.ui.microClassTitle : dict.ui.btnAI}</h2>
            {!microClassActive ? (
              <>
                <p style={{color:'#fff', fontSize:'clamp(16px, 4.5vw, 22px)', margin: 0}}>{qData.q}</p>
                <div style={ui.gridOptions}>
              ...
              <>
                <p style={{color:'#ffea00', fontSize:'clamp(16px, 4.5vw, 20px)', lineHeight:'1.5', margin: 0}}>{qData.micro}</p>
                <button style={ui.nextBtn('#0f0')} onClick={() => { setPhase("EXECUTION"); setMicroClassActive(false); }}>{dict.ui.btnContinue}</button>
              </>
            )}
          </div>
        </div>
      ...
      {phase === "SYNTHESIS" && (
        <div style={ui.overlay}>
          <div style={ui.dialogBox('#0f0')}>
            <h2 style={{color:'#0f0', letterSpacing:'3px', fontSize:'clamp(20px, 6vw, 35px)', margin: 0}}>{dict.ui.synthTitle}</h2>
            <p style={{fontSize:'clamp(16px, 4.5vw, 24px)', lineHeight:'1.5', color:'#fff', margin: 0}}>{dict.realWorld[missionIdx]}</p>
            <button style={ui.nextBtn('#0f0')} onClick={() => loadMission(missionIdx + 1)}>{dict.ui.btnNext}</button>
          </div>
        </div>
      )}
...
const ui = {
  ...
  centerScreen: { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100dvh', width: '100vw', background: 'radial-gradient(circle at center, #001122 0%, #000 100%)', zIndex: 1000, position: 'relative', padding: '20px', boxSizing: 'border-box', textAlign: 'center' },
  titleComplete: { color: '#0f0', fontSize: 'clamp(28px, 8vw, 60px)', letterSpacing: 'clamp(2px, 1vw, 8px)', textShadow: '0 0 40px #0f0', margin: 0, textAlign: 'center' },
  titleGlow: { color:'#00f2ff', fontSize:'clamp(32px, 10vw, 80px)', letterSpacing:'clamp(3px, 2vw, 15px)', textShadow:'0 0 50px rgba(0, 242, 255, 0.8)', margin:'0 0 30px 0', textAlign: 'center', fontWeight: '900' },
  btnHex: (c) => ({ padding:'clamp(15px, 4vw, 30px) clamp(30px, 8vw, 80px)', background:`linear-gradient(45deg, rgba(0,0,0,0.9), ${c}33)`, border:`3px solid ${c}`, color:c, fontSize:'clamp(16px, 5vw, 26px)', fontWeight:'900', cursor:'pointer', borderRadius:'15px', fontFamily:'Orbitron', transition:'all 0.3s ease', boxShadow: `0 0 30px ${c}55`, letterSpacing: '3px', width: '100%', maxWidth: '400px' }),

  // Header Nav (Volver / IA)
  topControls: { position: 'absolute', top: 'max(10px, env(safe-area-inset-top))', left: 0, width: '100%', padding: '0 clamp(10px, 4vw, 30px)', boxSizing: 'border-box', display: 'flex', justifyContent: 'space-between', zIndex: 500, pointerEvents: 'none', gap: '10px' },
  backBtn: { padding: 'clamp(10px, 3vw, 15px) clamp(15px, 4vw, 30px)', background: 'rgba(255,0,85,0.15)', border: '2px solid #ff0055', color: '#ff0055', cursor: 'pointer', borderRadius: '10px', fontFamily: 'Orbitron', fontWeight: '900', backdropFilter: 'blur(10px)', letterSpacing: '1px', transition: '0.3s', pointerEvents: 'auto', fontSize: 'clamp(12px, 3.5vw, 16px)' },
  aiBtn: { padding: 'clamp(10px, 3vw, 15px) clamp(15px, 4vw, 30px)', background: 'rgba(255,0,255,0.15)', border: '2px solid #ff00ff', color: '#ff00ff', cursor: 'pointer', borderRadius: '10px', fontFamily: 'Orbitron', fontWeight: '900', backdropFilter: 'blur(10px)', letterSpacing: '1px', transition: '0.3s', pointerEvents: 'auto', fontSize: 'clamp(12px, 3.5vw, 16px)' },

  // HUD Central (Niveles y Stats en Grid Automático)
  hud: { position: 'absolute', top: 'calc(max(10px, env(safe-area-inset-top)) + 65px)', left: '50%', transform: 'translateX(-50%)', zIndex: 100, width: '95%', maxWidth: '600px', display: 'flex', justifyContent: 'center', pointerEvents: 'none' },
  glassCard: { background: 'rgba(0,10,25,0.85)', border: '2px solid #00f2ff', padding: 'clamp(15px, 4vw, 30px)', borderRadius: '15px', backdropFilter: 'blur(15px)', boxShadow: '0 0 30px rgba(0,242,255,0.2)', width: '100%', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' },
  title: { color: '#fff', margin: 0, fontSize: 'clamp(18px, 5vw, 28px)', letterSpacing: '2px' },
  badge: { display: 'inline-block', marginTop: '10px', padding: '6px 15px', background: '#ff0055', color: '#fff', fontSize: 'clamp(12px, 3vw, 14px)', fontWeight: 'bold', borderRadius: '6px' },
  
  // Magia pura de CSS Grid: Si no caben, se apilan abajo (mobile), si caben, van al lado (desktop)
  ...
  dialogBox: (c) => ({ border: `2px solid ${c}`, background: 'rgba(0,0,0,0.95)', padding: 'clamp(20px, 6vw, 40px)', borderRadius: '20px', textAlign: 'center', width: '100%', maxWidth: '700px', boxShadow: `0 0 60px ${c}55`, maxHeight: '85dvh', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '15px', boxSizing: 'border-box' }),
  
  // Botones de Acción Modales (Grid Inteligente para móvil y PC)
  btnGroup: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px', width: '100%', marginTop: '10px' },
  gridOptions: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '15px', width: '100%', marginTop: '10px' },
  ...
  nextBtn: (c) => ({ padding: '15px 20px', background: c, border: 'none', color: '#000', fontSize: 'clamp(16px, 4.5vw, 22px)', fontWeight: 'bold', fontFamily: 'Orbitron', cursor: 'pointer', borderRadius: '10px', boxShadow: `0 0 30px ${c}`, width: '100%', minHeight: '60px', marginTop: '10px' }),
...
// Parches visuales (Ocultar scrollbar fea en móviles dentro del Modal)
