...
const LiveEquation = ({ mode, p, v, t }) => {
  ...
  if (mode === 'FREE') return (
    <div style={{display:'flex', flexDirection:'column', alignItems:'center', gap:'10px', fontSize:'clamp(16px, 2.5vw, 20px)', fontWeight:'bold'}}>
      <div><Var char="P" color={cP}/> · <Var char="V" color={cV}/> = nR · <Var char="T" color={cT}/></div>
      <div style={{fontSize:'clamp(12px, 2vw, 16px)', color:'#aaa'}}>{val(p, cP)} · {val(v, cV)} = k · {val(t, cT)}</div>
    </div>
  );
  if (mode === 'BOYLE') return (
    <div style={{display:'flex', flexDirection:'column', alignItems:'center', gap:'10px', fontSize:'clamp(16px, 2.5vw, 20px)', fontWeight:'bold'}}>
      ...
      <div style={{fontSize:'clamp(12px, 2vw, 16px)', color:'#aaa'}}>{val(p, cP)} · {val(v, cV)} = {k_boyle} (k)</div>
    </div>
  );
  if (mode === 'CHARLES') return (
    <div style={{display:'flex', flexDirection:'column', alignItems:'center', gap:'10px', fontSize:'clamp(16px, 2.5vw, 20px)', fontWeight:'bold'}}>
      ...
      <div style={{fontSize:'clamp(12px, 2vw, 16px)', color:'#aaa'}}><Fraction top={val(v, cV)} bottom={val(t, cT)} /> = {k_charles} (k)</div>
    </div>
  );
  if (mode === 'GAY_LUSSAC') return (
    <div style={{display:'flex', flexDirection:'column', alignItems:'center', gap:'10px', fontSize:'clamp(16px, 2.5vw, 20px)', fontWeight:'bold'}}>
      ...
      <div style={{fontSize:'clamp(12px, 2vw, 16px)', color:'#aaa'}}><Fraction top={val(p, cP)} bottom={val(t, cT)} /> = {k_gl} (k)</div>
    </div>
  );
  return null;
};
  ...
  return (
    <>
      ...
      {appState === 'GAME_SELECT' && (
        <div style={ui.screenGame}><div style={ui.hexBackground} /><button onClick={() => window.location.reload()} style={ui.resetBtnGame}>⬅ BACK</button>
          ...
          {activeQuiz && (
            <div style={ui.quizOverlay}>
              <div style={ui.quizBox}>
                ...
                <p style={{fontSize:'clamp(16px, 3vw, 22px)', margin:'clamp(15px, 3vw, 30px) 0', lineHeight:'1.5'}}>{activeQuiz.question}</p>
                ...
                {quizFeedback && (
                  <div style={{ padding: 'clamp(15px, 3vw, 30px)', background: quizFeedback.type === 'success' ? 'rgba(0,255,0,0.1)' : 'rgba(255,0,85,0.1)', border: `2px solid ${quizFeedback.type === 'success' ? '#0f0' : '#ff0055'}`, color: quizFeedback.type === 'success' ? '#0f0' : '#ff0055', fontSize: 'clamp(14px, 2.5vw, 18px)', borderRadius: '8px', textAlign: 'left' }}>
                    <h3 style={{marginTop: 0, fontSize: 'clamp(18px, 4vw, 24px)', textAlign: 'center'}}>{quizFeedback.type === 'success' ? t.correct : t.error}</h3>
                    <p style={{color: '#fff', fontSize: 'clamp(16px, 3vw, 20px)', textAlign: 'center'}}>{quizFeedback.text}</p>
                    
                    {quizFeedback.type === 'error' && activeQuiz.miniClass && (
                      <div style={{marginTop: 'clamp(15px, 3vw, 25px)', padding: 'clamp(10px, 2vw, 20px)', background: 'rgba(0,242,255,0.05)', borderLeft: '4px solid #00f2ff'}}>
                        <h4 style={{margin: '0 0 10px 0', color: '#00f2ff', letterSpacing: '1px'}}>{t.classHeader}</h4>
                        <p style={{margin: 0, color: '#ddd', fontSize: 'clamp(14px, 2.5vw, 18px)', lineHeight: '1.6'}}>{activeQuiz.miniClass}</p>
                      </div>
                    )}

                    <div style={{marginTop: 'clamp(15px, 3vw, 25px)', display: 'flex', justifyContent: 'center'}}>
                      {quizFeedback.type === 'success' ? (
                        <button onClick={closeQuiz} style={{...ui.solidCyberBtn, width: '100%'}}>{t.continue}</button>
                      ) : (
                        <button onClick={clearFeedback} style={{...ui.cyberBtn, width: '100%', borderColor: '#ff0055', color: '#ff0055', padding: '15px'}}>{t.tryAgain}</button>
                      )}
                    </div>
                  </div>
                )}
          ...
          {activeGame === 'REDOX_LAB' ? (
             ...
             <>
                ...
                <div className="game-panel-left" style={ui.leftPanel(isMobile)}>
                  ...
                  {/* Este subpanel se oculta en móviles muy pequeños si falta espacio, o se hace super compacto */}
                ...
                <div className="game-panel-right" style={ui.rightPanel(isMobile)}>
                  ...
                  {activeMode !== 'FREE' && (
                    <div style={{...ui.sectionBox, borderLeft:'3px solid #ff0055', background:'rgba(30,0,10,0.8)', flexShrink: 0, maxHeight: isMobile ? '15vh' : '25vh', overflowY: 'auto'}}>
                ...
                <Canvas camera={{ position: [0, 4, isMobile ? 22 : 15], fov: 45 }}>
                ...
                <div style={ui.controlPanel(isMobile)}>
                   ...
                   <div style={ui.hudControl(isCritical, phaseID, isMobile)}>
...
const ui = {
  ...
  centerBoxGame: { zIndex:10, display:'flex', flexDirection:'column', alignItems:'center', backdropFilter:'blur(8px)', padding:'clamp(20px, 5vw, 50px)', background:'rgba(0,5,15,0.7)', border:'1px solid rgba(0,242,255,0.2)', borderRadius:'4px', boxShadow:'0 0 50px rgba(0,0,0,0.8)' },
  titleGame: { color:'#00f2ff', fontSize:'clamp(30px, 6vw, 65px)', letterSpacing:'clamp(2px, 1vw, 8px)', textAlign:'center', margin:'0 0 30px 0', textShadow:'0 0 20px rgba(0,242,255,0.5)', fontWeight:900 },
  
  btnGridGame: { display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(120px, 1fr))', gap:'20px', width:'100%' },
  ...
  gameCard: { background: 'rgba(0,15,30,0.8)', border: '1px solid #00f2ff', padding: 'clamp(15px, 3vw, 30px)', borderRadius: '8px', cursor: 'pointer', transition: '0.3s', textAlign: 'center' },
  
  cyberBtn: { padding:'clamp(10px, 2vw, 15px) clamp(15px, 3vw, 20px)', background:'rgba(0,10,20,0.8)', border:'1px solid #005577', color:'#00f2ff', cursor:'pointer', fontSize:'clamp(12px, 2.5vw, 14px)', fontFamily:'Orbitron', fontWeight:'bold', transition:'0.2s' },
  solidCyberBtn: { padding:'clamp(10px, 2vw, 15px) clamp(20px, 4vw, 40px)', background:'linear-gradient(45deg, rgba(0,242,255,0.3), rgba(0,0,0,0.8))', borderLeft:'4px solid #00f2ff', color:'#fff', cursor:'pointer', fontSize:'clamp(14px, 3vw, 18px)', fontFamily:'Orbitron', fontWeight:'bold', width:'100%', margin:'0 auto', display:'block' },
  
  resetBtnGame: { position:'absolute', top: 'clamp(10px, 2vh, 25px)', left: 'clamp(10px, 2vw, 25px)', zIndex:100, padding:'clamp(8px, 1.5vw, 10px) clamp(15px, 3vw, 20px)', background:'rgba(0,0,0,0.5)', border:'1px solid #ff4444', color:'white', cursor:'pointer', fontFamily:'Orbitron', fontSize: 'clamp(12px, 2vw, 16px)' },
  ...
  // Paneles Laterales: En móvil se esconden con una clase CSS inyectada abajo, o se colapsan
  leftPanel: (isMobile) => ({ position:'absolute', top: isMobile ? '60px' : '80px', left: isMobile ? '10px' : '30px', zIndex:50, display: isMobile ? 'none' : 'flex', flexDirection:'column', gap:'15px', width:'clamp(200px, 25vw, 280px)', height: isMobile ? 'auto' : 'calc(100vh - 180px)' }),
  rightPanel: (isMobile) => ({ position:'absolute', top: isMobile ? '60px' : '80px', right: isMobile ? '10px' : '30px', zIndex:50, display: isMobile ? 'none' : 'flex', flexDirection:'column', gap:'15px', width:'clamp(220px, 28vw, 300px)', maxHeight: isMobile ? 'auto' : 'calc(100vh - 180px)', overflowY:'auto' }),
  
  sectionBox: { background:'rgba(0,10,20,0.7)', border:'1px solid rgba(0,85,119,0.5)', padding:'clamp(10px, 2vw, 15px)', backdropFilter:'blur(8px)', borderRadius: '8px' },
  panelTitle: { color:'#4488aa', margin:'0 0 10px 0', fontSize:'clamp(9px, 1.5vw, 10px)', letterSpacing:'3px' },
  searchInput: { width:'100%', padding:'8px', background:'rgba(0,0,0,0.5)', border:'1px solid #00f2ff', color:'#fff', fontFamily:'Orbitron', outline:'none', boxSizing:'border-box', flexShrink: 0 },
  pill: { flex:1, padding:'4px', fontSize:'clamp(8px, 1.5vw, 9px)', background:'transparent', color:'#888', border:'1px solid #333', cursor:'pointer', fontFamily:'Orbitron' },
  pillA: { flex:1, padding:'4px', fontSize:'clamp(8px, 1.5vw, 9px)', background:'rgba(0,242,255,0.2)', color:'#fff', border:'1px solid #00f2ff', cursor:'pointer', fontFamily:'Orbitron' },
  matBtn: { padding:'8px', background:'rgba(0,0,0,0.5)', color:'#00f2ff', border:'1px solid #005577', cursor:'pointer', fontFamily:'Orbitron', fontSize:'clamp(10px, 1.8vw, 11px)', textAlign:'left' },
  matBtnActive: { padding:'8px', background:'rgba(0,242,255,0.15)', color:'#fff', borderLeft:'3px solid #00f2ff', borderTop:'1px solid #00f2ff', borderRight:'1px solid #00f2ff', borderBottom:'1px solid #00f2ff', cursor:'pointer', fontFamily:'Orbitron', fontSize:'clamp(10px, 1.8vw, 11px)', fontWeight:'bold' },
  dataRow: { display:'flex', justifyContent:'space-between', fontSize:'clamp(9px, 1.5vw, 11px)', marginBottom:'6px', color:'#fff', borderBottom:'1px solid rgba(255,255,255,0.1)', paddingBottom:'3px' },
  modeGrid: { display:'grid', gridTemplateColumns:'1fr 1fr', gap:'5px' },
  modeBtn: { padding:'8px 4px', background:'rgba(0,0,0,0.5)', color:'#888', border:'1px solid #333', cursor:'pointer', fontFamily:'Orbitron', fontSize:'clamp(9px, 1.5vw, 10px)' },
  modeBtnA: { padding:'8px 4px', background:'rgba(255,234,0,0.1)', color:'#ffea00', border:'1px solid #ffea00', cursor:'pointer', fontFamily:'Orbitron', fontSize:'clamp(9px, 1.5vw, 10px)', fontWeight:'bold' },
  
  iaButton: { width:'100%', padding:'clamp(10px, 2vw, 15px)', background:'linear-gradient(45deg, #7b2cbf, #b5179e)', border:'2px solid #f72585', color:'#fff', cursor:'pointer', fontFamily:'Orbitron', fontSize:'clamp(12px, 2vw, 14px)', fontWeight:'bold', marginTop:'5px', boxShadow:'0 0 15px rgba(247, 37, 133, 0.5)', borderRadius: '8px' },
  
  // Controles Inferiores (Dashboard Central): Fluidos y envueltos
  controlPanel: (isMobile) => ({ position:'absolute', bottom: isMobile ? '10px' : '20px', left:'50%', transform:'translateX(-50%)', zIndex:10, display:'flex', flexWrap: 'wrap', justifyContent: 'center', alignItems:'center', gap:'clamp(10px, 2vw, 20px)', background:'rgba(0,5,15,0.85)', padding:'clamp(10px, 2vw, 15px) clamp(15px, 3vw, 30px)', borderRadius:'10px', border:'1px solid #00f2ff', boxShadow:'0 0 20px rgba(0,242,255,0.15)', backdropFilter:'blur(5px)', width: '95%', maxWidth: '900px' }),
  
  controlGroup: { display:'flex', flexDirection:'column', gap:'5px', alignItems: 'center' },
  controlLabel: (color) => ({ fontSize: 'clamp(8px, 1.5vw, 10px)', color: color, letterSpacing: '2px', textAlign: 'center', margin: 0, textShadow: `0 0 5px ${color}` }),
  actionBtn: (color) => ({ padding:'clamp(8px, 1.5vw, 12px) clamp(10px, 2vw, 15px)', background:`rgba(${color==='#ff0055'?'255,0,85':(color==='#00f2ff'?'0,242,255':'255,234,0')}, 0.1)`, border:`2px solid ${color}`, color:color, cursor:'pointer', fontWeight:'bold', fontFamily:'Orbitron', borderRadius: '4px', fontSize: 'clamp(10px, 2vw, 14px)' }),
  
  hudControl: (isCrit, phase, isMobile) => ({ background:'rgba(0,0,0,0.6)', padding:'clamp(5px, 1vw, 10px) clamp(15px, 3vw, 30px)', border:`2px solid ${phase==='plasma'?'#fff':(isCrit?'#ff0055':'#00f2ff')}`, textAlign:'center', minWidth:'clamp(100px, 20vw, 120px)', borderRadius:'5px', boxShadow: phase==='plasma'?'0 0 30px #fff':'', display: 'flex', flexDirection: isMobile ? 'row' : 'column', gap: isMobile ? '15px' : '0', alignItems: 'center' }),
  hudVal: (isCrit, baseColor, isMobile) => ({ fontSize: isMobile ? 'clamp(14px, 3vw, 18px)' : 'clamp(18px, 4vw, 22px)', fontWeight:'bold', color: isCrit ? '#ff0055' : baseColor, margin: isMobile ? '0' : '5px 0', textShadow:`0 0 10px ${baseColor}` }),
  
  quizOverlay: { position:'absolute', inset:0, background:'rgba(0,5,10,0.95)', zIndex:9999, display:'flex', alignItems:'center', justifyContent:'center', backdropFilter:'blur(15px)', padding: '20px' },
  quizBox: { background:'rgba(0,10,20,0.9)', border:'1px solid #00f2ff', padding:'clamp(20px, 5vw, 50px)', maxWidth:'1000px', width:'100%', maxHeight:'90dvh', overflowY:'auto', textAlign:'center', boxShadow:'0 0 100px rgba(0,242,255,0.2)', borderRadius: '15px' },
  quizGrid: { display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(200px, 1fr))', gap:'clamp(10px, 2vw, 15px)', marginBottom:'20px' },
  quizBtn: { padding:'clamp(15px, 3vw, 25px)', background:'rgba(255,255,255,0.05)', border:'1px solid #555', color:'#fff', cursor:'pointer', fontFamily:'Orbitron', fontSize:'clamp(14px, 3vw, 16px)', textAlign:'left', transition:'0.3s', borderRadius: '8px' }
};
...
if (typeof document !== 'undefined' && !document.getElementById("app-styles-mobile")) {
  ...
  styleSheet.innerText = `
    ...
    /* Clase auxiliar por si acaso */
    @media (max-width: 768px) {
       .game-panel-left, .game-panel-right { display: none !important; }
    }
  `;
